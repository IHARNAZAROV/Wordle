document.addEventListener("DOMContentLoaded", () => {
    ...
    const createScene = async () => {
        ...
        // loading vocab
        await loadFile('/assets/words_5_letters.txt');
    ...
    function gameLost() {
        window.alert(`Шеф, Все пропало! (слово было: ${keyWord.toUpperCase()})`);
    }
    ...
    function submitWord() {
        ...
        if (word.length !== 5) {
            shakeSquares();
            setTimeout(() => {
                window.alert('Слово должно состоять из пяти букв');
            }, 100)
            setTimeout(() => {
                resetAnimationClass();
            }, 500)
            ...
            shakeSquares();
            setTimeout(() => {
                window.alert('Такого слова я не знаю');
            }, 100)
            setTimeout(() => {
                resetAnimationClass();
            }, 500)
